...
OR should we instead keep the container running?

```bash
docker compose exec dev sh -c "wp core multisite-install --url=http://localhost:8080 --title=xyz --admin_user=root --admin_password=root --admin_email=first.lastname@test.com --skip-email && tail -f /dev/null"
```

IS there a need to bring up all the services?
